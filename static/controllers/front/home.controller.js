...
app.run(function($rootScope, $document, $log, $window) {
    ...
    if($document.scrollTop()>260){
        ...
        document.getElementById('mobileMenuID').className = 'mobile-nav-menu fix-it'
    }else{
        document.getElementById('menu-fixed').className = 'fixed-nav-menu';
        document.getElementById('mobileMenuID').className = 'mobile-nav-menu'
    }
...
app.controller('RotorController', ['$scope','$rootScope', '$interval','$document','$element','$attrs', '$http','$location', '$window', '$filter', '$log', function($scope, $rootScope, $interval, $document, $element, $attrs, $http, $location, $window, $filter,$log){
    ...
    $scope.startRot = function() {
        ...
        $interval(function () {
            ...
            $document.find('.rotor .slide.slide-' + $scope.iterbefore).removeClass('active');
            $document.find('.rotor .slide.slide-' + $scope.iter).addClass('active');


            if ($scope.iter == ($scope.slidecount - 1)) {
                $scope.iter = 0;
                $scope.iterbefore = $scope.slidecount - 1;
    ...
    $scope.changeSlide = function(i){
        ...
        $document.find('.rotor-cont .bg').removeClass('active');
        $document.find('.rotor-cont .bg.bg-' + i).addClass('active');
        $document.find('.rotor-cont .image').removeClass('active');
        $document.find('.rotor-cont .image.image-' + i).addClass('active');
        $document.find('.rotor-cont .title').removeClass('active');
        $document.find('.rotor-cont .title.title-' + i).addClass('active');
        $document.find('.rotor-cont .desc').removeClass('active');
        $document.find('.rotor-cont .desc.desc-' + i).addClass('active');
        $document.find('.rotor-cont .over-link').removeClass('active');
        $document.find('.rotor-cont .over-link.over-link-' + i).addClass('active');
        $document.find('.rotor-cont .btn-rot').removeClass('active');
        $document.find('.rotor-cont .btn-rot.btn-' + i).addClass('active');

        if (i == ($scope.slidecount - 1)) {
            $scope.iter = 0;
            $scope.iterbefore = $scope.slidecount - 1;
        } else if (i == 0) {
            $scope.iterbefore = 0;
...
app.controller('YoutubeController', ['$scope', '$interval','$document','$element','$attrs', '$http','$location', '$window', '$filter', '$log', function($scope, $interval, $document, $element, $attrs, $http, $location, $window, $filter,$log){

    $scope.clearPlayMovie = function(id){

        //console.log(angular.element(document.getElementById(id)));
        document.getElementById(id).src='';

    }

    $scope.addPlayMovie = function(id,link){
