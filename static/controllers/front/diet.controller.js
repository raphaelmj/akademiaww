...
app.controller('DietTabelController', ['$scope', '$rootScope', '$interval','$document','$element','$attrs', '$http','$location', '$window', '$filter', '$log', function($scope, $rootScope, $interval, $document, $element, $attrs, $http, $location, $window, $filter,$log){

    $scope.initData = function(){
        //console.log($scope.dietId);
    ...
    $scope.getDietData = function(){
        $http.get($scope.shopUrl+'/api/diet/'+$scope.dietId)
            .then(
                function(res){
                    ...
                    if(res.data.profiles.length>0) {
                        ...
                        $scope.data.selectDiet = $scope.data.profiles[0];
                        $scope.data.selectProfile = $scope.data.profiles[0].config[0];
                    }else{
                        $scope.data = null;
                    }
    ...
    $scope.$watch('data.selectDiet',function(nVal,oVal){
        if(nVal!=undefined) {
            $scope.data.selectDiet = null;
            $scope.data.selectDiet = nVal;
            $scope.data.selectProfile = nVal.config[0]
        }
    });

    $scope.$watch('data.selectProfile',function(nVal,oVal){
        //console.log(nVal);
        if(nVal!=undefined) {
            //$scope.data.selectProfile = $scope.data.selectDiet.config[0]
            console.log($scope.shopUrl+'/#/to/order?diet='+$scope.dietId+'&profile='+$scope.data.selectDiet.id+'&days='+$scope.data.selectProfile.days);
        }
    ...
    $scope.goToOrder = function(){
        ...
        //$window.location.href = $scope.shopUrl+'/#/to/order?diet='+$scope.data.diet.id+'&profile='+$scope.data.selectDiet.id+'&days='+$scope.data.selectProfile.days;
        $window.open($scope.shopUrl+'/#/to/order?diet='+$scope.dietId+'&profile='+$scope.data.selectDiet.id+'&days='+$scope.data.selectProfile.days); //'_blank', 'height=600,width=1400'
